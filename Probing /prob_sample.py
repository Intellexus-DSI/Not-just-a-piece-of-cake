...
def main():
    ...
    output_file = os.path.join(output_dir, f"{layer_wise}_{SPLIT}_{num_samples_str}_{model_name}_new.jsonl")
    output_file_vectors = os.path.join(output_dir, f"{layer_wise}_{SPLIT}_{num_samples_str}_{model_name}_vectors_new.pt")
    ...
    if NUM_SAMPLES > 0:
        ...
        # Cut smart, taking TARGET_IDIOMS_NUM idioms
        unique_idioms = data["idiom_base"].unique()
        if len(unique_idioms) > TARGET_IDIOMS_NUM:
            print(f"More than {TARGET_IDIOMS_NUM} unique idioms found. Sampling...")
            sampled_idioms = np.random.choice(unique_idioms, size=TARGET_IDIOMS_NUM, replace=False)
            data = data[data["idiom_base"].isin(sampled_idioms)]
            print(f"Sampled down to {len(data)} samples with {TARGET_IDIOMS_NUM} unique idioms.")
        else:
            print(f"Less than {TARGET_IDIOMS_NUM} unique idioms found. Using all {len(unique_idioms)} unique idioms.")
            data = data[data["idiom_base"].isin(unique_idioms)]

        # Randomly sample remaining2process samples
        if len(data) >= remaining2process:
            print(f"Data size before sampling: {len(data)}")
            # Randomly sample remaining2process samples
            ...
            # Stop
            raise ValueError(f"Not enough data to sample {remaining2process} samples. Only {len(data)} available.")
            return
